...
# common/passwords.php is re-locked to 640 after the global chmod above loosens it to 664:
...
ssh -S "$SOCKET" -t "$REMOTE" "sudo chown -R www-data:developers $REMOTE_PATH/ && sudo find $REMOTE_PATH/ -type d -exec chmod 2775 {} + && sudo find $REMOTE_PATH/ -type f -exec chmod 664 {} + && sudo chmod 640 $REMOTE_PATH/common/passwords.php"
