...
**Brewer URLs: one brewer per homepage, not one per domain** (since v4.1.0, 2026-09-24). `brewer.domainName` is the URL's host, `www.` stripped and lowercased; it is indexed but **not unique**. A domain's root URL belongs to at most one brewer and no two brewers may hold the same page, but any number may hold distinct sub-pages of one domain — an acquired brand that carries on under its own name lives on a page of its owner's site (Whetstone Beer Co. at `northchair.com/whetstone-beer/`) beside the owner's record at the root. `BrewerUrl.class.php` is the one place host and page keys are computed: `host()`, `path()`, `pageKey()` (trailing slash and `/index.html` collapsed, case-insensitive, fragment dropped, query kept), `isRoot()`, and `conflict()`, which classifies every brewer on the host and returns the colliding row with `match: root|page`. `Brewer::validateURL()` calls it (error 182, two messages) after `UrlCheck` has canonicalised the URL; `unique_url` remains as the byte-identical backstop. Because staff rights come from the email-domain match, **a shared domain shares staff by design** — an `@owner.com` address is staff on every brewer that stores `owner.com`. **Offline regression test: `php tests/brewer-url.php`**; `conflict()` is covered by `tests/brewer-lead.php` and the Postman "Shared Domain" folder (BR-238..245).
...
- `/login`
- `/review/claim` (POST), `/review` (GET, `?needs_decision=1`), `/review/{id}` (GET, PATCH), `/brewer/{id}/review` (GET) — Admin-only queue and record for the brewer review loop (`Review.class.php`). `claim` returns every brewer whose latest review carries an unactioned decision (regardless of `count`, so an answer never waits on the size of a run) plus the next `count` brewers holding a URL (`count` defaults to 10 when omitted; `0` claims only the decided rows), and stamps `brewer.claimedBy/claimedAt` in one transaction (`FOR UPDATE SKIP LOCKED`; claims expire on the read side after 4h); the response's `decided` says how many of the rows carry a decision. **A claim can also name brewers:** `brewer_ids` (1..50 UUIDs, deduplicated case-insensitively) holds exactly those rows, after the decided rows and before the queue's `count` (which defaults to 0, not 10, when `brewer_ids` is present). Named rows skip the queue order **and the URL requirement** (a brewer whose URL was cleared is exactly the kind a human points an agent at), and the ones not held come back in `skipped` with a `reason` of `unknown` or `claimed` (plus `claim_expires_at`) instead of failing the call, so a partial claim is still usable; every claim response carries `skipped`, empty when nothing was named. Posting a review has never checked the claim, so a targeted review was always possible; the named claim is what makes it safe against a concurrent queue run and puts it inside the brief's procedure. `POST /review/{brewer_id}` inserts a `brewer_review` row, sets `brewer.reviewedAt`, clears the claim, and on `url_verdict: ok` refreshes the URL-health columns the way the cron's ok branch does. **A list never ships the audit trail:** `GET /review` and `GET /brewer/{id}/review` return the stored `sourcesCount`/`changesCount` columns as `sources_count` and `changes_count` in place of `sources`, `changes` and `notes` — the three unbounded columns — which only `GET /review/{id}` carries (a write answers through `get()`, so POST and PATCH still echo the full shape). A page of 100 reviews each holding thousands of change entries is tens of MB to decode and re-encode, past `memory_limit`, and `notes` is the same problem one size down: 20,000 characters JSON-escape to ~240 KB as emoji, so 500 rows is a worst case of the same shape. `question` and `decision` stay in a list — the check-in list is unreadable without them and both are capped at 500 characters. A list shows what happened; fetch the review to read what was written and why. **The counts are stored columns, never `JSON_LENGTH()` at read time** — a JSON-derived expression in the select list of a filesorting query is declared wide enough to blow `sort_buffer_size`, which 500s the endpoint regardless of row count (`migrations/2026-09-25-review-stored-counts.sql`); `tests/review.php` guards it both structurally and under a 32 KB buffer. The caps on a posted review are 10,000 `changes` entries (2 MB), 1,000 `sources` (512 KB) and 20,000 characters of `notes`; **the count cap and the byte guard on a list must always be raised together**, or the count admits a body the guard refuses. `notes` is `mediumtext` because 20,000 characters can exceed what `text` holds. `needs_decision` rows are the human check-in list; `PATCH /review/{id}` records the `decision`, which the next claim of that brewer returns as `last_review`. The same PATCH also amends `notes` and `question` on a row whose `decision` is still null (plain replace, same caps as POST), so a bad post is corrected in place instead of becoming a permanent row; once decided both are immutable (409), and `question` is only amendable on a row posted with `needs_decision: true`. Schema: `catalog-beer-mysql/migrations/2026-09-08-brewer-review.sql`. **Regression test: `php tests/review.php`** — needs a local MySQL (creates and drops a scratch `cb_review_test` database from the canonical schema; no network, no secrets file); run it for any change to the class or the two review tables. Not in the public API docs, like the other admin-only endpoints.
- `/brewer-lead` (POST, GET), `/brewer-lead/claim` (POST), `/brewer-lead/{id}` (GET, PATCH) — Admin-only lead queue for the review loop (`BrewerLead.class.php`): breweries a review met that the catalog does not hold, recorded as a name, a place and the page they were read on, so they are researched later through the six steps instead of created thin now. **A lead is not a record and nothing in the table is published.** Two stored states (`open`/`closed`); the wire `status` derives `claimed` from `claimedAt` within `Review::CLAIM_TTL`, exactly as the brewer claim works. `POST` dedups server-side in every status: a `url` that is a brewer's homepage, a page a brewer holds, or any page on a host whose root a brewer holds is refused with 409 and the `brewer_id` (`BrewerUrl::conflict()` with `$rootClaimsPages`; a root URL on a host held only by sub-page brewers is a genuine lead — the owner of an acquired brand); lead-vs-lead dedup stays host-level, so two leads on one host merge into one row; a host or `nameKey`+`sub_code` match on an existing lead returns that row with 200, bumps `lastSeenAt`, appends the request's `source_url` to `sources` (cap 20), and merges nothing else (closed rows are the negative cache). `nameKey` is `SearchQuery::brewerNameKey()`, the same generic-word test search uses for a name match. The `url` is validated for syntax only and **never fetched** — a lead must be able to carry a dead or not-yet-live site. `claim` returns every open lead carrying a `decision` (regardless of `count`) then the next `count` queued rows oldest first, skipping rows with an open question or a future `recheck_after`. `PATCH` is one operation per call, named by its field: `resolution` (`created`/`duplicate` require an existing `brewer_id`, the other two refuse it; closes the row), `recheck_after` (defer, `note` required, back to the queue, clears a carried decision), `needs_decision` + `question` (ask; the row leaves the claim until decided), `decision` (the human's answer; the next claim leads with the row). Any open row accepts a resolve/defer/ask whether or not the caller holds the claim; a closed row is 409 unless the body carries `reopen: true` — the whole correction mechanism, since every key here is admin and the server cannot tell one from another. `brewerID` is a FK `ON DELETE SET NULL`. Spec: `scratch/brewer-leads-spec.md`; schema: `catalog-beer-mysql/migrations/2026-09-09-brewer-lead.sql`. **Regression test: `php tests/brewer-lead.php`** — local MySQL, scratch `cb_lead_test` database from the canonical schema; run it for any change to the class or the table. Not in the public API docs or the published skill; the agent learns these routes from `catalog-beer-review/BRIEF.md`.
...
- `/usage`, `/usage/my-usage` — no response anywhere carries an API key: `/usage/my-usage` and `/billing` don't echo the caller's own (it authenticated with it; echoing it only creates a leak path when agents print responses), and the admin list `GET /usage` identifies each key by `user_id` + `api_key_last4` (the frontend's `admin/usage.php` groups on that pair)
