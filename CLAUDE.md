...
- `/usage`, `/usage/my-usage` — neither `/usage/my-usage` nor `/billing` echoes the caller's key back (it authenticated with it; echoing it only creates a leak path when agents print responses)
